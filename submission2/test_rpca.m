...
img = imread('.\data\6.jpg');% convert given file into RGB
img = rgb2gray(img);
img = im2double(img);

img_corrupted = img;
img_corrupted = imnoise(img_corrupted,'salt & pepper', 0.02); % Corrupt image
img_corrupted(2:30:end) = NaN; % Missing image
...
error = img - img_final;
RMS_error = sqrt(sum(sum(error.^2))/(size(img,1)*size(img,2)));
disp(RMS_error);

fprintf(1, 'ws=%d\tlambda=%f\trank(L)=%d\tcard(S)=%d\terr=%f\n', ...
       ws, lambda, rank(L), nnz(S), RMS_error);